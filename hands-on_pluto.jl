...
# ╔═╡ 513fadd6-bf19-4cb1-acbc-904cd8cd8982
md"""
# Julia and Python on Google Colab
##### [Hiroyuki Chihara](https://fiomfd.github.io/) (University of the Ryukyus, Okinawa Island, Japan)

## GitHub (notebooks and some links)
## [https://github.com/fiomfd/hands-on](https://github.com/fiomfd/hands-on)

##### links to Google Colab, a handout, html versions of notebooks and etc.
"""

# ╔═╡ 67132c33-ed30-4b08-a84e-b9b083f19332
begin
	Downloads.download("https://raw.githubusercontent.com/fiomfd/hands-on/refs/heads/main/data/github.png","github.png")
	QR1=load("github.png")
	QR=imresize(QR1, (200, 200))
end

# ╔═╡ 8b9fc6c3-160a-451c-91ea-b34d73f93217
md"""
#### Topics
...
# ╔═╡ d7e9f676-5369-4a83-9ba7-70850d296f40
...
# ╔═╡ cd520fdd-628e-417e-8bef-7d36facd38e6
begin
	x=ones(Int64,150)
    x[1]=m
for n=2:150
...
# ╔═╡ c49f69fa-a502-430e-bc7a-223182c18c5c
...
QQQ4=plot(colorview(RGB,RGBII),
        ...
        yticks=false, 
        grid=false);
plot(QQQ1,QQQ2,QQQ3,QQQ4,size=(700,500),layout=(2,2))
end

# ╔═╡ 1d8fe203-85b9-481b-9319-895e9575d04e


# ╔═╡ 41a75ead-557d-4917-9027-dea6c7c06d16
md"""
#### 3. RGB movies
"""

# ╔═╡ cabf8a58-dfcf-46a5-ab78-bf14c749d9a7
begin
...
# ╔═╡ 60683298-503e-4278-8906-ab3908122ade
...
# ╔═╡ eb1afa41-9ce2-41f9-9c5c-3a1ae4b814d0


# ╔═╡ 0c3b43ec-23bd-43a2-a6dc-2aa67e841d7a
md"""
#### 6. Public data and visualization
...
# ╔═╡ ebbb143d-33f1-4e48-8fbf-e49c735e9374
md"""
...
# ╔═╡ 2b319962-2cfc-4aac-b85d-15870db12fd3
md"""
...
# ╔═╡ 14db3cff-c861-48b9-b9aa-74c95e382c1f


# ╔═╡ 4191e9e9-4578-4b67-ad5c-6eb5e5d8c640
md"""
#### 7. Central limit theorem for the binomial distribution $B(n,p)$
...
# ╔═╡ 2eb74b64-2592-4887-afad-05d281ad5abc
md"""
 p = $(@bind p1 Slider(0.1:0.1:0.9, show_value=true, default=0.5))
 $nbsp $nbsp
 n = $(@bind n1 Slider(1:1800, show_value=true, default=0))
"""

# ╔═╡ c09d5570-3d14-4105-9c8f-25fcc52ca5dd
begin
	...
	for l=0:n1
		z2[l+max(100,n1)+1]=Float64(sqrt(n1*p1*(1-p1))*factorial(BigInt(n1))*p1^l*(1-p1)^(n1-l)/factorial(BigInt(l))/factorial(BigInt(n1-l)));
	end
	
	z1 = zeros(length(x1));
	c1 = Int64((length(x1)-1)/2);
	for k=1:c1
		z1[c1+k]=z2[max(100,n1)+2+Int64(floor(n1*p1+(k-1)/200*sqrt(n1*p1*(1-p1))))];
		z1[c1+1-k]=z2[max(100,n1)+1+Int64(floor(n1*p1-(k-1)/200*sqrt(n1*p1*(1-p1))))];
	end
end

# ╔═╡ 31361cc8-73a3-400f-89f0-94a7036bdcab
begin
    plot(f1,
...
# ╔═╡ 87dafcce-7c2f-4b76-a191-b18f49230908
begin
	Kmax=14
	...
	for k=0:Kmax
	    for l=1:301
	        s13[k+1,l]=(-1)^k*y[l]^(2*k+1)/factorial(big(2*k+1));
        end
	end
	S13=zeros(Kmax+1,301);
	S13[1,:]=s13[1,:];
	for l=1:301
	    for k=2:Kmax+1
		    S13[k,l]=S13[k-1,l]+s13[k,l];
	    end
	end
end

# ╔═╡ 323dea67-83fa-45da-8993-efb0b3f7f574
md"""
#### 8. Taylor series of $\sin{x}$
...
K = $(@bind l3 Slider(0:Kmax, show_value=true))

"""

# ╔═╡ bf45c341-0527-4c71-b1ec-b77a79a2d2eb
begin
	h3=S13[l3+1,:];
...
# ╔═╡ 7109fcb2-e456-4554-a601-802d2485f239


# ╔═╡ de9e2641-677b-43e7-8ad8-4709c33745fd
md"""
#### 9. Fourier series of a step function and a sawtooth function
...
# ╔═╡ db7259e7-5ebc-42aa-8c67-62ee1b13bc71
begin
	# Set Maximun number of terms M2
	M2=50;
	...
	# S_{2K-1}[g]
	x2 = range(-0.1, 1.1, length = 481);
	S2=ones(M2+1,481)/2;
	#l=10
	for k=2:M2+1
		for l=1:481
			S2[k,l]=S2[k-1,l]+2*sin(2*(2*k-3)*pi*x2[l])/(2*k-3)/pi;
		end
	end
	...
	for k=2:M2+1
		for l=1:481
			S3[k,l]=S3[k-1,l]-sin(2*(k-1)*pi*x2[l])/(k-1)/pi;
		end
	end

end

# ╔═╡ c286d5a3-5630-4d6d-af47-fd65170dc6d1
md"""
 $K$ = $(@bind K2 Slider(0:M2, show_value=true))
"""

# ╔═╡ 9274a6fb-0285-4097-b0d1-84723285a31a
begin
...
G1=plot(x2,S2[K2+1,:],
		...
		xlim=(-0.15,1.15),
		ylim=(-1/8,9/8),
		title="Step Function and its Fourier Series, K=$K2",
        titlefontsize=9,
...
G2=plot(x2,S3[K2+1,:],
		...
		xlim=(-0.15,1.15),
		ylim=(-1/8,9/8),
		title="Sawtooth Function and its Fourier Series, N=$K2",
        titlefontsize=9,
		xticks = ([0 1/2 1;], [0,1/2,1]),
		yticks = ([0 1/2 1;], [0,1/2,1]),
		xlabel=L"x",
...
plot(G1,G2, layout=(1,2), size=(700,300))

end

# ╔═╡ 989218dd-b245-43db-9efb-2bb745357c16


# ╔═╡ 2b9ef7ce-3442-41f7-8b51-65a7850c37ba
md"""
#### 10. Weyl's equidistribution theorem
...
# ╔═╡ e9f391a6-3eaa-492c-991a-d7a3dfd11218
MWeyl=250;

# ╔═╡ 26d9349e-e4bf-424f-ba54-1a27295ac424
md"""
...
# ╔═╡ 604939ca-ce62-4b9e-9b26-b1eecd25eeb8
begin
...
# ╔═╡ 66feb603-8281-4e71-9086-0fe7e1dcc2fa
begin
